...
| 시작점 | **[시작 배치] 도구로 제작자가 배치** (2026-09-06). 클릭 위치에 시작 마커, 다시 클릭하면 이동. 캔버스 안쪽 여백(x 0.5u·y 0.6u)으로 자동 클램프, 골과 3u 이상. 미배치면 검증·제출 불가 |
| 기본 경계 | **바닥·왼쪽 벽·오른쪽 벽·천장** 네 변 모두 고정 콜라이더로 항상 존재 (캔버스 밖으로 나갈 수 없음 — 2026-09-06 QA 로 천장·오른쪽 개방 폐기). 캔버스에 표시되지만 편집 불가 |
...
| 시작·골 최소 거리 | 3u 이상 (패타임 0.2초짜리 맵 방지) — 나중에 놓는 쪽에서 검사 |
| 실행취소 깊이 | 50단계 |
...
interface ILoadableMap {
    void Load(MapData map);   // 콜라이더 + 렌더러 + 기본 경계(사방) + 골 존 생성
    void Unload();
    MapData Current { get; }
...
- [x] 시작·골 이동 배치 정상 동작, 미배치 시 검증·제출 차단
