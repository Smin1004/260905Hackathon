...
| 항목 | 초기값 (튜닝 대상) |
|---|---|
| 물리 | `Rigidbody2D` (Dynamic) + `BoxCollider2D` |
| 좌우 이동 속도 | 5 u/s |
| 점프력 | 10 (2단 점프 없음) |
| 중력 스케일 | 2 |
| 지면 판정 | 발 하단 짧은 레이캐스트 (Raycast 2D) |
| 조작 | 이동: **A/D 또는 ←/→** · 점프: **W 또는 ↑** · 리스폰: **R** · 스페이스바는 기능 없음 |
| 입력 시스템 | 신 Input System — `InputSystem_Actions.inputactions`에 `Move`/`Jump`/`Respawn` 액션 정의. Active Input Handling이 New/Both 어느 쪽이든 동작 |

- 지면 판정 기반 점프 (공중 점프 불가) — 코어 룰상 점프는 중요한 제약 대상이라 판정을 명확히 유지
